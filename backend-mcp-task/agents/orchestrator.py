    ...
    def execute_custom_flow(self, initial_context: Dict[str, Any], 
        ...
        for agent in sequential_agents:
            ...
            try:
                ...
                if callback:
                    callback(agent.name, "started", None, {"step": current_step, "total_steps": total_steps})
                result = agent.execute(self.context)
                self.context[agent.name] = result
                if callback:
                    callback(agent.name, "completed", result, {"step": current_step, "total_steps": total_steps})
            except Exception as e:
                print(f"[Orchestrator] Agent {agent.name} failed: {str(e)}")
                traceback.print_exc()
                self.context[agent.name] = {"error": str(e)}
                if callback:
                    callback(agent.name, "failed", {"error": str(e)}, {"step": current_step, "total_steps": total_steps})
        ...
        if parallel_agents:
            ...
            if callback:
                callback("ParallelOptimizationPhase", "started", None, {"step": current_step, "total_steps": total_steps, "agents": [a.name for a in parallel_agents]})
            ...
            if callback:
                callback("ParallelOptimizationPhase", "completed", parallel_results, {"step": current_step, "total_steps": total_steps})
